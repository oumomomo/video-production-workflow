...
4. Self-check in the order of the user's highest-frequency failure modes: baseline preservation, script match, visual reference contract, rhythm, composition, graphic-element purpose, aesthetic quality, then text density. These eight are hard stops. Every line, border, underline, path, or connector must carry a named structural, relational, directional, measurement, separation, or real-interface role. Delete it when the editor cannot explain what it connects, what it means, and what understanding is lost without it. Apply a first-glance rejection gate before showing any visual: reject unsourced default AI packaging or an unfinished draft internally. Do not infer AI-looking style from isolated surface traits such as a zoomed product interface, negative space, native small text, or brand colors; judge whether each choice has a real source and a specific narrative, composition, or timing role. AI review may fail aesthetic or text density, but may not mark either one passed; passing evidence must come from the user or a named human editor.
...
- Do not use lines, borders, underlines, paths, or connector buses as empty technical decoration. Do not repeat the same hierarchy with several kinds of line. If linework makes the shot read like a generic architecture diagram, express the event through the objects themselves moving, merging, generating, replacing, or changing state. Necessary linework inherited from an approved real interface or brand system remains valid.
